    ...
    <main>
        <div class="container-fluid">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Peserta belum disetujui</li>
            </ol>
            <div class="card shadow-lg">
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-12">
                            <h3 class="text-primary mb-3">Peserta Belum Disetujui</h3>
                            <div class="flash-data" data-pesandata="<?= Flasher::pesan(); ?>" data-tipedata="<?= Flasher::type(); ?>"></div>
                            <div class="row">
                                <div class="col-lg-4">
                                    <form action="<?= BASEURL . '/mentor/belumdisetujui' ?>" method="post">
                                ...
                                <table class="table table-bordered">
                                    <thead class="thead-light">
                                        <tr>
                                            ...
                                            <th scope="col" width="50px" class="text-center">Image</th>
                                        ...
                                        <tbody>
                                            ...
                                            <?php else : ?>
                                                <?php foreach($data['peserta'] as $peserta) : ?>
                                                    <?php if($peserta['is_actived'] === 0 && $peserta['mentor'] === $_SESSION['session']['username']) : ?>
                                                        <tr>
                                                            <th scope="row"><?= ++$data['awalData'] ?></th>
                                                            <td><img src="<?= BASEURL . '/asset/img/' . $peserta['image'] ?>" width="100%" height="47px" class="rounded-circle" style="object-fit: cover;" alt="profil"></td>
                                                            ...
                                                            <td class="text-center">
                                                                <a href="<?= BASEURL . '/mentor/accpeserta/' . $peserta['id'] ?>" class="btn btn-success accPeserta" title="Setujui"><i class="fas fa-check"></i> Setujui</a>
                                                                <a href="<?= BASEURL . '/mentor/detailPesertaBelumDisetujui/' . $peserta['id'] ?>" class="btn btn-primary" title="Detail"><i class="fa-solid fa-circle-info fa-sm"></i></a>
                                                            </td>
                                                        </tr>
                                                    <?php endif ?>
                                                <?php endforeach; ?>
                            ...
                            <ul class="pagination pagination-sm justify-content-center">
                                <?php if($data['hlmAktif'] > 1) : ?>
                                    ...
                                    <li class="page-item"><a class="page-link" href="<?= BASEURL . '/mentor/belumdisetujui/' . $prev ?>">Previous</a></li>
                                <?php endif ?>

                                <?php for($i = 1; $i <= $data['jmlHalaman']; $i++) : ?>
                                    <?php if($i == $data['hlmAktif']) : ?>
                                        <li class="page-item active"><a class="page-link" href="<?= BASEURL . '/mentor/belumdisetujui/' . $i ?>"><?= $i ?></a></li>
                                    <?php else : ?>
                                        <li class="page-item"><a class="page-link" href="<?= BASEURL . '/mentor/belumdisetujui/' . $i ?>"><?= $i ?></a></li>
                                    <?php endif ?>
                                <?php endfor ?>

                                <?php if($data['hlmAktif'] < $data['jmlHalaman']) : ?>
                                    <?php $next = $data['hlmAktif'] + 1; ?>
                                    <li class="page-item"><a class="page-link" href="<?= BASEURL . '/mentor/belumdisetujui/' . $next ?>">Next</a></li>
                                <?php endif ?>
                            </ul>
                            </nav>
                        </div>
